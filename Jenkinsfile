    ...
    stages {
        stage('Build') {
            steps {
                ...
                writeFile file: "src/micro/makefile", text: readFile("src/micro/makefile").replaceAll(/F_CPU.*\n/,"F_CPU=${env.F_CPU}\n")
                sh 'make build' 
                archiveArtifacts artifacts: "src/micro/makefile, src/micro/bin/Ardwiino.hex, src/config/config.h", fingerprint: true 
            }
        }
    }
}

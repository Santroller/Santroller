...
private:
    ...
    uint8_t m_cmd_ff_buf[0x0654];
    uint8_t m_data_ff_buf[0x1000];
    CFG_TUSB_MEM_ALIGN uint8_t m_cmd_buf[TUH_EPSIZE_BULK_MAX];
    // RX bulk aggregation is disabled, so one message per transfer bounds this at a single MPDU.
    CFG_TUSB_MEM_ALIGN uint8_t m_data_buf[TUH_EPSIZE_BULK_MAX];

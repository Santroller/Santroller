...
typedef struct
{
    ...
    uint8_t blueCymbal : 1;              // y, triangle, yellow
    uint8_t blue : 1;              // x, square, blue

    uint8_t kick2 : 1;    // leftShoulder, orange, l1
    uint8_t kick : 1;  // rightShoulder
    uint8_t l2 : 1;    // l2
    uint8_t r2 : 1;      // r2
    uint8_t : 2;
    uint8_t back : 1;   // back, select
...
} __attribute__((packed)) FestivalProDrumsLayerRB_Data_t;
...
} __attribute__((packed)) FestivalProDrumsLayerGH_Data_t;
typedef struct
{
    uint8_t reportId;

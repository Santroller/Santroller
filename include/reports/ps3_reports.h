...
typedef struct {
    ...
    uint16_t accelY;  // Up/down acceleration (gravity)
    uint16_t gyro;    // Left/right instantaneous rotation (yaw)
} __attribute__((packed)) PS3GamepadDrum_Data_t;
typedef struct {
    uint8_t reportId;
    uint8_t : 8;
    uint8_t back : 1;             // select
...
typedef struct {
    ...
    uint8_t l2 : 1;    // l2 - whammy
    uint8_t r2 : 1;      // r2 - overdrive
    uint8_t kick2 : 1;    // l1 (leftShoulder) - strum up
    uint8_t kick : 1;  // r1 (rightShoulder) - strum down

    uint8_t blue : 1;              // triangle (y) - blue
    uint8_t green : 1;   // circle (b) - orange
    uint8_t greenCymbal : 1;  // cross (a)
    uint8_t blueCymbal : 1;              // square (x) - yellow
    ...
    uint16_t accelY;  // Up/down acceleration (gravity)
    uint16_t gyro;    // Left/right instantaneous rotation (yaw)
} __attribute__((packed)) PS3FestivalProDrumLayer_Data_t;
typedef struct {
    // Button bits
    uint8_t x : 1;  // square
    uint8_t a : 1;  // cross

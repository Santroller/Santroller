...
inline void x360_to_universal_report(const uint8_t *data, uint8_t len, uint8_t sub_type, USB_Host_Data_t *usb_host_data) {
    ...
    switch (sub_type) {
        ...
        case XINPUT_GUITAR_ALTERNATE: {
            ...
            usb_host_data->blue |= report->blue;
            usb_host_data->orange |= report->orange;
            usb_host_data->pedal |= report->pedal;
            if (report->tilt) {
...
inline void universal_report_to_x360(uint8_t *data, uint8_t len, uint8_t sub_type, const USB_Host_Data_t *usb_host_data) {
    ...
    switch (sub_type) {
        ...
        case GUITAR_HERO_DRUMS: {
            ...
            if (usb_host_data->orangeVelocity) {
                report->orangeVelocity = usb_host_data->orangeVelocity;
            }
            if (usb_host_data->kickVelocity) {
        ...
        case ROCK_BAND_PRO_KEYS: {
            ...
            report->pedal |= usb_host_data->pedal;
            if (usb_host_data->pedalAnalog) {
                report->pedalAnalog = usb_host_data->pedalAnalog << 1;
            }
            if (usb_host_data->touchPad) {
                report->touchPad = usb_host_data->touchPad << 1;
            }
            if (usb_host_data->velocities[0]) {
                report->velocities[0] = usb_host_data->velocities[0];
            }
            if (usb_host_data->velocities[1]) {
                report->velocities[1] = usb_host_data->velocities[1];
            }
            if (usb_host_data->velocities[2]) {
                report->velocities[2] = usb_host_data->velocities[2];
            }
            if (usb_host_data->velocities[3]) {
                report->velocities[3] = usb_host_data->velocities[3];
            }
            if (usb_host_data->velocities[4]) {

 ...
 */
// === MODIFIED ===
uint8_t readFromI2C(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
    uint8_t i;
    ...
    */
    ...
    if ( i2cMasterBufferIndex < length )
        length = i2cMasterBufferIndex;
 ...
 */
uint8_t writeToI2C(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
    uint8_t i;
    ...
    */
    ...
    while(wait && (TWI_MTX == i2cState)){
      continue;
         ...
         if ( i2cError == 0xFF )            return 0;	// success
    else if ( i2cError == TW_MT_SLA_NACK )  return 2;	// error: address send, nack received
    else if ( i2cError == TW_MT_DATA_NACK ) return 3;	// error: data send, nack received

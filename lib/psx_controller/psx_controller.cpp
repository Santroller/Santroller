...
PSXController::PSXController(SPIMasterInterface *interface, uint8_t csPin, uint8_t attPin, uint8_t ackPin) : interface(interface), csPin(csPin), attPin(attPin), ackPin(ackPin), connected(false) {
...
void PSXController::tick() {
    ...
    if (type == PSX_GUITAR_HERO_CONTROLLER && to_us_since_boot(get_absolute_time()) - last < 3000) {
        return;
    }
    ...
    if (!connected) {
        ...
        if (isDualShock2Reply(ps2Data)) {
            ...
            if ((~ps2Data[3]) & (1 << 7)) {
                type = PSX_GUITAR_HERO_CONTROLLER;
            } else {
                type = PSX_DUALSHOCK_2_CONTROLLER;
            }
        } else if (isDualShockReply(ps2Data)) {
            if ((~ps2Data[3]) & (1 << 7)) {
                type = PSX_GUITAR_HERO_CONTROLLER;
            } else {
                type = PSX_DUALSHOCK_1_CONTROLLER;
            }
        } else if (isFlightStickReply(ps2Data)) {
            type = PSX_FLIGHTSTICK;

...
#define INIT_TMP_SERIAL_TO_USB(tmp_ptr)                                                                \
    asm(                                                                                               \
        "lds %A[tmp], %[readPtr]\n\t" /* (1) Copy read pointer into lower byte*/ /* Outputs */         \
        : [tmp] "=&e"(tmp_ptr) /* Pointer register, output only*/                /* Inputs */          \
...
#define READ_BYTE_FROM_BUF(data_ptr, tmp_ptr)                                                                          \
    asm(                                                                                                               \
        ...
        "ld %[data] , %a[tmp] +\n\t" /* (2) Load next data byte, wraps around 255 */ /* Outputs */                     \
        : [data] "=&r"(data_ptr),                                                    /* Output only */                 \
          [tmp] "=e"(tmp_ptr) /* Input and output */                                 /* Inputs */                      \
        : "1"(tmp_ptr))

#define INIT_TMP_USB_TO_SERIAL(tmp_ptr)                                                                                    \
    asm(                                                                                                                   \
        "ldi %B[tmp], 0x02\n\t"                                                         /* (1) Force high byte to 0x200 */ \
        "lds %A[tmp], %[writePtr]\n\t" /* (1) Load USBtoUSART_WritePtr into low byte */ /* Outputs */                      \
        : [tmp] "=&e"(tmp_ptr) /* Pointer register, output only*/                       /* Inputs */                       \
...
#define WRITE_BYTE_TO_BUF(data_ptr, tmp_ptr)                                                                          \
    asm(                                                                                                              \
        ...
        "andi %A[tmp], 0x7F\n\t" /* (1) wrap around pointer, 128 bytes */ /* Outputs */                               \
        : [tmp] "=e"(tmp_ptr) /* Input and output */                      /* Inputs */                                \
        : "0"(tmp_ptr),                                                                                               \
          [data] "r"(data_ptr))
...
#define WRITE_ARRAY_TO_BUF(tmp_ptr, buf_ptr, length) \
    {                                                \
        ...
        do {                                         \
            ...
            data = *(buf++);                         \
            WRITE_BYTE_TO_BUF(data, tmp_ptr);        \
        } while (--len);                             \
    }

...
BandHeroDrumDevice::BandHeroDrumDevice(const DeviceReloadState* state, proto_BandHeroDrumDevice device, uint16_t id) :
    MidiDevice(state, id, false, m_midi_buffers.config()),
    m_band_hero_drum(this, device.i2c.block, device.i2c.sda, device.i2c.scl, device.i2c.clock),
    m_device(device)
{
}

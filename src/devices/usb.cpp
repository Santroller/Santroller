...
USBHostHardwareDevice::USBHostHardwareDevice(proto_UsbHostDevice device, uint16_t id) : UsbHostInterface(0, 0, id), m_device(device)
{
    ...
    printf("host_devices before: %d\r\n", host_devices.size());
    for (auto it = host_devices.begin(); it != host_devices.end();)
    {
        if (!tuh_connected(it->first))
        {
            printf("erase: %d\r\n", it->first);
...
void UsbHostInterface::update(bool full_poll, bool send_events)
{
    ...
    if (!m_fetched_name) {
        ...
        memcpy(event.event.usb.name, m_name, sizeof(m_name));
        resend = !HIDConfigDevice::send_event(event);
    }
    if (send_events && (full_poll || resend || !m_sent_type))
    {
        m_sent_type = true;
        proto_Event event = {which_event : proto_Event_usb_tag, event : {usb : {m_id, m_subtype, port: m_dev_addr}}};
        memcpy(event.event.usb.name, m_name, sizeof(m_name));
...
uint16_t usbh_open(uint8_t rhport, uint8_t dev_addr, tusb_desc_interface_t const *desc_itf, uint16_t max_len)
{
    ...
    {
        printf("insert: %d\r\n", dev_addr);
        host_devices.emplace(dev_addr, std::make_shared<UsbHostDevice>(dev_addr, usb_host_id));
    }
...
bool usbh_xfer_cb(uint8_t dev_addr, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
    if (ep_addr & 0x80)
    {
        return host_devices[dev_addr]->host_devices_by_endpoint_in[ep_addr & (~0x80)]->xfer_cb(ep_addr, result, xferred_bytes);
    }

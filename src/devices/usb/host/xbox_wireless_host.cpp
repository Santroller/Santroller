...
struct mt76_dev mt76_device;

static XboxWirelessHost* g_wireless_host = nullptr;

extern "C" void xbox_wireless_create_controller_interface(uint8_t controller_idx, uint8_t subtype)
{
    if (g_wireless_host)
    {
        g_wireless_host->create_controller_interface(controller_idx, subtype);
    }
}

extern "C" void xbox_wireless_remove_controller_interface(uint8_t controller_idx)
{
    if (g_wireless_host)
    {
        g_wireless_host->remove_controller_interface(controller_idx);
    }
}

extern "C" xbox_controller_t* xbox_get_controller(uint8_t index)
{
    if (!g_wireless_host) {
        return NULL;
    }
    
    auto controller = g_wireless_host->get_controller_interface(index);
    if (controller) {
...
extern "C" int xbox_adapter_send_gip_packet(uint8_t controller_idx, const uint8_t *data, uint16_t len)
{
    if (!g_wireless_host) {
        return -1;
    }
    
    auto controller = g_wireless_host->get_controller_interface(controller_idx);
    if (controller) {
        return controller->send_gip_packet(data, len);
    }
    
    return -1;
}

extern "C" void xbox_adapter_process_gip_data(const uint8_t *data, uint16_t len)
{
    if (len < 4) {
        return;
    }
    ...
    auto controller_interface = g_wireless_host->get_controller_interface(controller_idx);
    
    if (!controller_interface) {
        if (len >= 4 && data[0] == 0x02) {
            printf("Controller %d arrival detected, creating interface\n", controller_idx);
            g_wireless_host->create_controller_interface(controller_idx, 0xFF);
            controller_interface = g_wireless_host->get_controller_interface(controller_idx);
        } else {
            printf("Ignoring packet for controller %d (no interface, not arrival)\n", controller_idx);
            return;
        }
    }
    
    if (controller_interface) {
        controller_interface->process_gip_data(data, len);
    }
}

XboxWirelessHost::XboxWirelessHost(uint8_t dev_addr, uint8_t interface, uint16_t id) 
    : UsbHostInterface(dev_addr, interface, id)
    , m_mt76_dev(&mt76_device)
    , m_adapter_initialized(false)
    , m_firmware_loaded(false)
    , m_radio_initialized(false)
...
{
    if (g_wireless_host == this)
    {
        g_wireless_host = nullptr;
    }
    ...
    while (endpoints--)
    {
        ...
        if (desc_ep->bEndpointAddress & 0x80)
        {
            ...
            printf("XboxWirelessHost: Bulk IN endpoint 0x%02X, size=%d\r\n", 
                   intf->m_ep_in, intf->m_ep_in_size);
        ...
        else
        {
            ...
            printf("XboxWirelessHost: Bulk OUT endpoint 0x%02X, size=%d\r\n", 
                   intf->m_ep_out, intf->m_ep_out_size);
            TU_VERIFY(tuh_edpt_open(dev_addr, desc_ep), nullptr);
        }
    }
...
bool XboxWirelessHost::xfer_cb(uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
    if (ep_addr & 0x80 && result != XFER_RESULT_FAILED)
    {
        if (xferred_bytes > 0)
        {
            printf("XboxWirelessHost: Received %lu bytes on endpoint 0x%02X\r\n", 
                   xferred_bytes, ep_addr);
            
            wireless_process_data(m_ep_in_buf, xferred_bytes);
        }
    ...
    {
        printf("XboxWirelessHost: Bulk OUT complete on endpoint 0x%02X\r\n", ep_addr);
    }
...
void XboxWirelessHost::update(bool full_poll, bool send_events)
{
    ...
    if (m_radio_initialized && !m_pairing_enabled && (now - m_init_start_time) > 3000)
    {
        printf("XboxWirelessHost: Enabling pairing mode\r\n");
        
        if (mt76_set_pairing(m_mt76_dev, true) == 0)
        {
            printf("XboxWirelessHost: Pairing mode enabled (LED should blink)\r\n");
            m_pairing_enabled = true;
            
            DeviceManager::instance().remove_enumerating_usb_device(this);
            DeviceManager::instance().add_assignable_usb_device(host_devices[m_dev_addr]->host_devices_by_itf[m_interface]);
        }
    }

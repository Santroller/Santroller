...
PowerGigGuitarAxisMapping::PowerGigGuitarAxisMapping(proto_Mapping mapping, std::unique_ptr<Input> input, uint16_t id, uint32_t profile) : AxisMapping(mapping, std::move(input), id, profile, mapping.mapping.rbAxis == PowerGigGuitarWhammy)
{
}
...
void PowerGigGuitarAxisMapping::update_wii(uint8_t *buf)
{
    if (m_centered)
    {
        return;

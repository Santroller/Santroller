...
void handle_bt_rumble(uint8_t rumble_left, uint8_t rumble_right) {
    ...
    if (deviceType == DJ_HERO_TURNTABLE && rumble_right == RUMBLE_SANTROLLER_EUPHORIA_LED) {
    ...
    if ((consoleType == XBOXONE) && report_id != BLUETOOTH_REPORT) {
        ...
        if ((consoleType == XBOX360 || consoleType == WINDOWS) && report_id != BLUETOOTH_REPORT) {
            ...
            if (id == PS4_LED_RUMBLE_ID) {
...
#endif
            } else if (DEVICE_TYPE_IS_INSTRUMENT) {
                if (id == PS3_REPORT_ID && data[1] == SANTROLLER_LED_ID) {
                ...
                } else if (deviceType == DJ_HERO_TURNTABLE && id == DJ_LED_ID) {

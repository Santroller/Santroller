...
void ps3_to_universal_report(const uint8_t *data, uint8_t len, uint8_t sub_type, USB_Host_Data_t *usb_host_data) {
    ...
    switch (sub_type) {
        ...
        case GUITAR_HERO_GUITAR: {
            ...
            if (sub_type == GUITAR_HERO_GUITAR_WT) {
...
uint8_t universal_report_to_ps3(uint8_t dpad, uint8_t *data, uint8_t console_type, uint8_t sub_type, const USB_Host_Data_t *usb_host_data) {
    if (sub_type != GAMEPAD) {
        PS3Dpad_Data_t *report = (PS3Dpad_Data_t *)data;
        report->dpad = dpad;
    ...
    switch (sub_type) {
        case GAMEPAD: {
            ...
            if (SWAP_SWITCH_FACE_BUTTONS && console_type == SWITCH) {
                report->b = usb_host_data->a;
                report->a = usb_host_data->b;
                report->x = usb_host_data->y;
                report->y = usb_host_data->x;
            }

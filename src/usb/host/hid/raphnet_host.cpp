...
std::shared_ptr<UsbHostInterface> RaphnetHost::open(std::shared_ptr<UsbHostDevice> list, tusb_desc_interface_t const *itf_desc, uint16_t max_len, uint16_t vid, uint16_t pid, uint16_t revision, HID_ReportInfo_t *info)
{
    ...
    auto intf = std::make_shared<RaphnetHost>(dev_addr, itf_desc->bInterfaceNumber, list->m_id, info);
...
bool RaphnetHost::tick_digital(UsbButtonType type)
{
    return false;
}
uint16_t RaphnetHost::tick_analog(UsbAxisType type)
{
    return 0;
}